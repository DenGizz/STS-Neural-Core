...
def SendPictureToService():
    import json
    with open('/content/connection/service_server_connection_info', 'r') as f:
     x = f.read()
     j = json.loads(x)
     os.system('sshpass -p 777 scp -o StrictHostKeyChecking=no -P ' + j["port"] + ' /content/drive/MyDrive/Style-Transfer-Network/image_output/result/result.png root@' + j["domain"] + ':/content/connection')

def step_callback_func(x):
  global step 
  step += 1
...
def minimize_with_lbfgs(sess, net, optimizer, init_img):
  ...
  step = 0;
  i = 0
  while i < math.trunc(args.max_iterations / args.print_iterations):

     optimizer.minimize(sess, step_callback=step_callback_func)
  
     output_img = sess.run(net['input'])
     out_dir = os.path.join(args.img_output_dir, args.img_name)
     maybe_make_directory(out_dir)
     #img_path = os.path.join(out_dir, str(step) + 'step.png')
     img_path = os.path.join(out_dir, 'result.png')
     SendPictureToService()
